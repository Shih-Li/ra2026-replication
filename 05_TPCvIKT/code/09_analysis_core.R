# 09_analysis_core.R
# Shared analysis functions translated from the three Stata analysis variants.
# No code executes when this file is sourced.
...
run_hh_balance_classes_receipt <- function(hh) {
  ...
  if ("n_class" %in% names(hh)) {
    ...
    if ("class_o" %in% names(hh)) {
      idx <- true_idx(hh$class_o == 1)
      hh$n_class_noORG[idx] <- hh$n_class_noORG[idx] - 1
    }
...
prepare_individual_analysis <- function(individual, hh) {
    ...
    if (rhs %in% names(d)) {
      if (!nm %in% names(d)) d[[nm]] <- d[[rhs]]
        ...
        if (is.character(d[[nm]])) {
          z <- d[[nm]]; z[true_idx(z == "")] <- NA_character_
          u <- d[[rhs]]; if (is.character(u)) u[true_idx(u == "")] <- NA_character_
  ...
  bad_cal_idx <- true_idx(d$age >= 0 & d$age <= 7 & (d$cal < 200 | d$cal > 2000))
  for (v in existing(d, c("vitc","vita","vitb12","ca","prot","fe","zn"))) d[[v]][bad_cal_idx] <- NA_real_
  if ("cal" %in% names(d)) d$cal[bad_cal_idx] <- NA_real_
  ...
  bad_h_idx <- true_idx(bad_h)
  if ("height" %in% names(d)) d$height[bad_h_idx] <- NA_real_
  if ("peso" %in% names(d)) d$peso[bad_h_idx] <- NA_real_
  idx <- true_idx(d$age == 3 & !is.na(d$peso) & abs(d$peso - 140.25) < 1e-8)
  d$height[idx] <- NA_real_; d$peso[idx] <- NA_real_
  idx <- true_idx(d$age == 0 & dplyr::between(d$peso, 48, 49))
  d$peso[idx] <- NA_real_
  idx <- true_idx(d$age == 0 & dplyr::between(d$peso, 80, 81))
  d$peso[idx] <- NA_real_
  ...
  idx <- true_idx(d$age == 0 & dplyr::between(d$age_month, 6, 7)); d$pct_rda_cal[idx] <- d$cal[idx] / 650
  idx <- true_idx(d$age == 0 & dplyr::between(d$age_month, 8, 11)); d$pct_rda_cal[idx] <- d$cal[idx] / 850
  idx <- true_idx(dplyr::between(d$age, 1, 3)); d$pct_rda_cal[idx] <- d$cal[idx] / 1300
  idx <- true_idx(dplyr::between(d$age, 4, 6)); d$pct_rda_cal[idx] <- d$cal[idx] / 1800
  idx <- true_idx(d$age >= 12); d$pct_rda_cal[idx] <- d$cal[idx] / 2200
...
run_individual_analysis <- function(individual, hh) {
  ...
  for (amax in c(6,1)) {
    ...
    # Stata's US CDC `ba` (BMI-for-age) chart is defined only for ages 2--20.
    # The source do-file nevertheless loops over zbmi again for ages 0--1, where
    # the outcome is structurally missing. Keep zbmi in the 0--6 panel, but omit
    # it from the 0--1 panel rather than treating a structurally empty regression
    # as a data failure.
    health_outcomes <- c("peso","height","sick","anemia","zw4a","zh4a")
    if (amax >= 2) health_outcomes <- c(health_outcomes, "zbmi")
    
    run_panel_regressions(d, health_outcomes, sub,
...
make_attrition_panel <- function(hh_attrit) {
  ...
  for (v in c("exp_total","exp_food","exp_inkind","exp_n_inkind","exp_nfood")) {
    ...
    idx <- true_idx(d$etapa == 2 & d$attriter == 1)
    d[[y]][idx] <- NA_real_
    by <- paste0("b_", y)
