...
make_table1_panel <- function(d, panel, sample_mask) {
  ...
  rows <- lapply(balance_vars, function(v) {
    ...
    # For the Gender row, the original Stata loop mechanically runs
    #     reg lb_genero lb_genero, r
    # which is a degenerate self-regression. Stata drops the RHS variable;
    # R warns repeatedly because the response also appears on the RHS.
    # The difference-by-gender p-value is not defined for the gender variable
    # itself, so leave that table cell blank/NA.
    if (identical(v, "lb_genero")) {
      p <- NA_real_
    } else {
...
run_tables_1_to_8 <- function(root) {
  out_dir <- ensure_dir(file.path(root, "output", "tables", "main"))
  ...
  message("Tables 1-8 written to output/tables/main.")
  invisible(d)
}


run_tables_1_to_8(ROOT)
