...
check_packages <- function() {
  ...
  if (length(missing)) {
    stop(
      ...
      "\nRun source('code/01_setup.R') first, then rerun code/00_master.R.",
      call. = FALSE
    )
  }
}
...
require_files <- function(paths) {
  ...
  if (length(missing)) {
    stop(
      ...
      "\n\nSee README.md for the required folder layout.",
      call. = FALSE
    )
  }
}
...
require_cols <- function(data, cols, context = "data") {
  ...
  if (length(miss)) {
    hints <- vapply(miss, function(v) {
      cand <- names(data)[startsWith(names(data), v)]
      if (length(cand) == 1L) paste0(v, " -> ", cand) else ""
    }, character(1))
    hints <- hints[nzchar(hints)]
    hint_text <- if (length(hints)) paste0(
      "\nPossible Stata abbreviation match(es): ", paste(hints, collapse = "; ")
    ) else ""
    stop(
      "Missing column(s) in ", context, ": ", paste(miss, collapse = ", "),
      hint_text, call. = FALSE
    )
  }
  invisible(TRUE)
}

# Stata allows a unique prefix of a variable name to be used as an abbreviation.
# R does not.  For translated code, materialize an alias whenever an exact name
# is absent but the requested Stata prefix identifies exactly one stored column.
materialize_stata_abbrevs <- function(data, refs, context = "data") {
  mapping <- character(0)
  for (ref in unique(refs)) {
    if (ref %in% names(data)) next
    candidates <- names(data)[startsWith(names(data), ref)]
    if (length(candidates) == 1L) {
      data[[ref]] <- data[[candidates]]
      mapping[ref] <- candidates
    } else if (length(candidates) == 0L) {
      stop(
        "Could not resolve Stata variable abbreviation '", ref,
        "' in ", context, ".", call. = FALSE
      )
    } else {
      stop(
        "Ambiguous Stata variable abbreviation '", ref, "' in ", context,
  ...
  if (length(mapping)) {
    message(
      "Resolved Stata variable abbreviation(s): ",
      paste(paste0(names(mapping), " -> ", unname(mapping)), collapse = "; ")
    )
  }
  attr(data, "stata_abbrev_mapping") <- mapping
  data
}
...
# Stata's vce(robust)/, robust corresponds to an HC1-style finite-sample
# correction. Recent sandwich versions warn whenever a fitted design has
# hat values numerically equal to 1, even for HC0/HC1. In this project the
# many randomization-stratum indicators can trigger the same warning on each
# Romano-Wolf refit, creating hundreds of duplicate warnings. Suppressing the
# warning does not change the HC1 covariance returned by sandwich.
vcov_hc1 <- function(fit) {
  suppressWarnings(sandwich::vcovHC(fit, type = "HC1"))
}
...
# Shared analysis-variable construction used by main and appendix tables.
