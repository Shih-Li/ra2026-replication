# ============================================================
# 02_data_prep.R
# Reconstruct the analysis-ready dataset from six raw WBHS files
# ============================================================
...
build_wbhs_dataset <- function(root) {
  ...
  # The Stata source uses names such as cs1_mea. These are legal Stata
  # abbreviations for uniquely matching longer stored variable names (for
  # example cs1_mean). Materialize equivalent aliases before using them in R.
  pic_groups <- list(cs = paste0("cs", 1:3, "_mea"), hi = paste0("hi", 1:3, "_mea"),
                     ap = paste0("ap", 1:3, "_mea"), sr = paste0("sr", 1:3, "_mea"))
  x <- materialize_stata_abbrevs(
    x, unlist(pic_groups, use.names = FALSE), context = "PICTURES_anon.dta variables"
  )
