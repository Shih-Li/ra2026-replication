...
# Builds the person-level panel from raw Baseline and 1st Followup modules.
# Generated files are written to data/intermediate/ and data/processed/ only.
...
build_individual <- function(b_hh_sample = NULL, f_hh_sample = NULL, transfers = NULL, save = TRUE) {
  # Stata `replace ... if ...` treats missing conditions as false.
  # R logical subscripts may contain NA, so use explicit TRUE positions.
  ...
  for (mod in list(health, tobacco, anthro, recall, blood)) {
    m <- stata_merge(individual, mod, by = c("pid", "etapa"), update = TRUE)
    m$._merge <- NULL
    individual <- m
  }
  ...
  if (!is.null(transfers) && all(c("cve_viv", "etapa", "pal_bene_linea") %in% names(transfers))) {
    ...
    individual <- stata_merge(pal, individual, by = c("cve_viv", "etapa"), update = TRUE)
  ...
  if ("tipo_ind" %in% names(individual)) {
    individual <- individual |>
      dplyr::filter(!(as_num(tipo_ind) %in% c(2, 3) & dplyr::between(age, 3, 98))) |>
